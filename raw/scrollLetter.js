 ...
 *
 * Options:
 * - offset (number): Percentage offset (0-100) at which the scroll animation begins. Default: 0.
 * - scrollTriggerClass (string): CSS class for elements that act as scroll triggers. (Required; e.g., ".scroll-trigger")
 * - parentClass (string): CSS class for the parent element containing the text. (Optional)
 * - zielKlasse (string): CSS class for target text elements to animate. (Required)
 * - initialBrightness (number): Initial opacity of words (0 to 1). Default: 0.15.
 * - targetBrightness (number): Final opacity when fully visible (0 to 1). Default: 1.
 * - transitionTime (string): Duration of the opacity transition (e.g., "0.3s"). Default: "0.3s".
 * - easing (string): CSS easing function (e.g., "ease", "ease-in-out"). Default: "ease".
 ...
 * - background (string): CSS background image value to assign to spans. (Optional)
 * - fallbackMobileBackground (string): Fallback background image for mobile devices. (Optional)
