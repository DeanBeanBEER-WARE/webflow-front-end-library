 ...
 *
 * Options:
 * - hTag (string): Heading tag to use (default: 'h1').
 * - addClass (string): Additional class(es) for the text element (required).
 * - textArray (Array<string>): Array of texts to display (required).
 * - targetParent (string): ID of the parent element for the typewriter (required).
 * - randomText (boolean): If true, texts are selected randomly (default: false).
 * - caretFrequency (number): Blink frequency of the caret in ms (default: 1000).
 * - endHold (number): Time in ms to hold the full text before deleting (default: 2000).
 * - startHold (number): Time in ms to wait after deleting before typing the next text (default: 500).
 * - characterHold (number): Time in ms between typing each character (default: 200).
 * - deleteHold (number): Time in ms between deleting each character (default: 100).
