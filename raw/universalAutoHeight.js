 ...
 * ToggleHeight Class
 *
 * Enables smooth toggling of element height (e.g., for FAQs or accordions) with configurable triggers, targets,
 * optional rotation elements, and customizable transition options.
 *
 * Options:
 * - parentSelector (string): CSS selector for the container (optional).
 * - triggerSelector (string): CSS selector for the element that triggers the toggle (required).
 * - targetSelector (string): CSS selector for the element whose height is toggled (required).
 * - rotateSelector (string): CSS selector for an element to rotate during toggling (optional).
 * - isOpen (boolean): Whether the target is open by default (default: false).
 * - duration (number): Transition duration in ms (default: 300).
 * - closeOthers (boolean): If true, opening one target closes others (default: true).
 * - easingMode (string): CSS easing function (default: 'ease').
 * - fontSizeMultiplier (number): Multiplier for collapsed height (default: 1).
 * - expandedRotation (number): Rotation angle in degrees when expanded (default: 90).
 *
 ...
 *     }
 *   ];
 *   new ToggleHeight(toggleConfigs);
 * });
 *
 * @version 1.0.0
 * @license MIT
