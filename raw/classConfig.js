 ...
 *
 * Options (applies to each configuration / FilterInstance):
 * - eventName (string): The event to listen for (e.g., "click", "hover", "scrollInView"). (Required)
 * - transitionTime (string): Duration of the transition (e.g., "0.5s"). Default: "0s".
 * - removeTransitionTime (string|null): Transition duration when removing classes. (Optional)
 * - easingMode (string): CSS easing function (e.g., "ease", "ease-in"). Default: "ease".
 * - entryThreshold (number): Percentage (0-100) at which classes are added. Default: 0.
 * - exitThreshold (number|null): Percentage (0-100) at which classes are removed. (Optional)
 * - parentElement (string|HTMLElement): Selector or element for the container of target elements.
 *      (If string, it may be a class selector (e.g., ".parent") or an ID selector (e.g., "#parent").)
 ...
 * - debounce (number): Delay in milliseconds for debouncing the event handling. Default: 0.
 ...
 * - triggerAttribut (string): Attribute name used for pairing trigger elements. (Optional)
 * - parentAttribute (string): Attribute name used for pairing parent elements. (Optional)
 * - callback (Function|null): Function executed after classes are added or removed. (Optional)
 * - addClasses (Array<string>): Array of class names to add.
 * - removeClasses (Array<string>): Array of class names to remove.
 * - topAddClasses (Array<string>): Array of classes to add at the top level.
 *
 * Example:
 *
 * const classConfigs = [
